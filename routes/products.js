...
router.get("/", async (req, res) => {
  try {
    ...
    if (pet) {
      where.push('(p.pet_type = ? OR p.pet_type = "all")');
      params.push(pet);
    }
    if (search) {
    ...
    // Мін/макс ціна для повзунка (завжди по всіх активних)
    const [[priceRange]] = await db.query(
      `SELECT MIN(price) AS minPrice, MAX(price) AS maxPrice FROM products WHERE is_active = 1`,
    );
...
router.post("/:id/reviews", async (req, res) => {
  try {
    ...
    if (!author_name || !rating)
      return res.status(400).json({ error: "Імʼя та оцінка обов'язкові" });
